...
def test(data,
    ...
    # for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(testloader, desc=s)):
        ...
        with torch.no_grad():
            ...
            out_all, Contrastive_Loss = model(img_rgb, img_ir, augment=augment) 
            out = out_all[0]
            train_out = out_all[1]
            ...
            if compute_loss:
                ...
                # _, loss_items = compute_loss([x.float() for x in train_out], targets, Contrastive_Loss, Flag = False)
                # loss[0] += loss_items[0]
                # loss[1] += loss_items[1]
                # loss[2] += loss_items[2]
                loss += compute_loss([x.float() for x in train_out], targets, Contrastive_Loss, Flag = False)[1][:3]
