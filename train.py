...
def train(hyp, opt, device, tb_writer=None):
    ...
    if pretrained:
        ...
        print("ckpt['model'].state_dict().keys():", ckpt['model'].state_dict().keys()) ######
        model = Model(opt.cfg or ckpt['model'].yaml, ch=3, nc=nc, anchors=hyp.get('anchors')).to(device)  # create
...
def train_rgb_ir(hyp, opt, device, tb_writer=None):
    ...
    if pretrained:
        ...
        # #####################################################
        # # ####################### yolov5l -》 yolov5l_fusion_transformer_m3fd

        # # 遍历模型B的state_dict
        # for keyB in model.state_dict():
        #     # # 解析keyB中的层次信息
        #     # parts = keyB.split('.')
        #     # layer_id = int(parts[1])
            
        #     if keyB in ckpt['model'].state_dict():
        #         model.state_dict()[keyB] = ckpt['model'].state_dict()[keyB]
        #         # print("layer {} ok".format(keyB))
                
        #     # else:
        #     #     print(f"Key '{keyB}' not found in checkpoint's state_dict.")
        # #####################################################
        exclude = ['anchor'] if (opt.cfg or hyp.get('anchors')) and not opt.resume else []  # exclude keys
        state_dict = ckpt['model'].float().state_dict()  # to FP32
        state_dict = intersect_dicts(state_dict, model.state_dict(), exclude=exclude)  # intersect
    ...
    #     # Optimizer
    #     if ckpt['optimizer'] is not None:
    #         print("ckpt['optimizer']:",ckpt['optimizer'].keys())
    #         optimizer.load_state_dict(ckpt['optimizer'])
    #         best_fitness = ckpt['best_fitness']
    ...
    if rank in [-1, 0]:
        testloader, testdata = create_dataloader_rgb_ir(test_path_rgb, test_path_ir,imgsz_test, batch_size * 2, gs, opt,  # testloader
                                       hyp=hyp, cache=opt.cache_images and not opt.notest, rect=True, rank=-1,
                                       world_size=opt.world_size, workers=opt.workers, 
                                       pad=0.5, prefix=colorstr('val: ')) # batch_size * 2 pad=0.5 
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            imgs = imgs.to(device, non_blocking=True).float() / 255.0  # uint8 to float32, 0-255 to 0.0-1.0
            imgs_rgb = imgs[:, :3, :, :]
            imgs_ir = imgs[:, 3:, :, :] # torch.Size([8, 3, 640, 640])
            ...
            with amp.autocast(enabled=cuda):
                ...
                # loss, loss_items = compute_loss(pred, targets.to(device))  # loss scaled by batch_size
                pred, contrastiveLoss = model(imgs_rgb, imgs_ir)  # forward
                loss, loss_items = compute_loss(pred, targets.to(device), contrastiveLoss)  # loss scaled by batch_size
                if rank != -1:
                    loss *= opt.world_size  # gradient averaged between devices in DDP mode
                if opt.quad:
                    loss *= 4.
        ...
        if rank in [-1, 0]:
            ...
            if not opt.notest or final_epoch:  # Calculate mAP
                ...
                results, maps, times = test.test(data_dict,
                                                 ...
                                                 wandb_logger=wandb_logger,
                                                 compute_loss=compute_loss,
                                                 is_coco=is_coco) # , half_precision=False
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='runs/train/exp143/weights/best.pt', help='initial weights path') # 'runs/train/exp7/weights/best.pt' 'yolov5l.pt' exp46 73 141
    parser.add_argument('--cfg', type=str, default='./models/transformer/yolov5l_fusion_transformer_M3FD_fuse3_fourier.yaml', help='model.yaml path') # yolov5l_fusion_transformer_M3FD_fuse3_fourier
    parser.add_argument('--data', type=str, default='./data/multispectral/M3FD_8_2.yaml', help='data.yaml path')
    ...
    print("global rank:", opt.global_rank)
