...
# class LoadImages_Two:  # for inference
#     def __init__(self, path1, path2, img_size=640, stride=32):
#         # os.path.abspath(p)返回p的绝对路径
#         p1 = str(Path(path1).absolute())  # os-agnostic absolute path
#         p2 = str(Path(path2).absolute())  # os-agnostic absolute path
...
#             if '*' in p2:
#                 files1 = sorted(glob.glob(p1, recursive=True))  # glob
#                 print("No file1, let file2 be both")
#                 # 如果path是一个文件夹，使用glob获取全部文件路径
...
#                 print("No file1, let file2 be both")
#             else:
#                 raise Exception(f'ERROR: {p1} or {p2} does not exist')
...
#         else:
#             if '*' in p1:
#                 files2 = sorted(glob.glob(p1, recursive=True))  # glob
#                 print("No file2, let file1 be both")
#                 # 如果path是一个文件夹，使用glob获取全部文件路径
#             elif os.path.isdir(p1):
#                 files2 = sorted(glob.glob(os.path.join(p1, '*.*')))  # dir
#                 print("No file2, let file1 be both")
...
#         self.files1 = images1 + videos1  # 整合图片和视频路径到一个列表
#         self.files2 = images2 + videos2  # 整合图片和视频路径到一个列表
...
#                 else:
#                     path1 = self.files1[self.count]
#                     self.new_video(path1)
#                     ret_val, img0_1 = self.cap.read()

#                     path2 = self.files2[self.count]
#                     self.new_video(path2)
#                     ret_val, img0_2 = self.cap.read()

#             # 当前读取的帧数
#             self.frame += 1
#             print(f'video {self.count + 1}/{self.nf} ({self.frame}/{self.frames}) {path1}: ', end='')
#             print(f'video {self.count + 1}/{self.nf} ({self.frame}/{self.frames}) {path2}: ', end='')
...
#             # 读取图片
#             self.count += 1
#             img0_1 = cv2.imread(path1)  # BGR
#             img0_2 = cv2.imread(path2)  # BGR

#             assert img0_1 is not None, 'Image Not Found ' + path1
#             assert img0_2 is not None, 'Image Not Found ' + path2

#             print(f'image {self.count}/{self.nf} {path1}: ', end='')
#             print(f'image {self.count}/{self.nf} {path2}: ', end='')


#         # Padded resize
#         # 对图片进行resize+pad
#         img1 = letterbox(img0_1, self.img_size, stride=self.stride)[0]
#         img2 = letterbox(img0_2, self.img_size, stride=self.stride)[0]
...
#         dataset1 = path1, img1, img0_1, self.cap

#         img2 = np.ascontiguousarray(img2)
#         dataset2 = path2, img2, img0_2, self.cap
...
#         return self.nf, self.nf  # number of files
...
def img2label_paths(img_paths):
    ...
    # print("img_paths_list:", img_paths_list)
    if "RGBimages" in img_paths_list[0]:
        sa, sb = os.sep + 'RGBimages' + os.sep, os.sep + \
            'labels' + os.sep  # /images/, /labels/ substrings
    elif "IRimages" in img_paths_list[0]:
        sa, sb = os.sep + 'IRimages' + os.sep, os.sep + \
            'labels' + os.sep  # /images/, /labels/ substrings
    elif "FOGimages" in img_paths_list[0]:
        sa, sb = os.sep + "FOGimages" + os.sep, os.sep + \
            'labels' + os.sep  # /images/, /labels/ substrings
    elif "U2Fuseimages" in img_paths_list[0]:
        sa, sb = os.sep + "U2Fuseimages" + os.sep, os.sep + \
            'labels' + os.sep  # /images/, /labels/ substrings
    elif "DenseFuseimages" in img_paths_list[0]:
        sa, sb = os.sep + "DenseFuseimages" + os.sep, os.sep + \
            'labels' + os.sep  # /images/, /labels/ substrings
    elif "PIAFusionimages" in img_paths_list[0]:
        sa, sb = os.sep + "PIAFusionimages" + os.sep, os.sep + \
            'labels' + os.sep  # /images/, /labels/ substrings
    # print("sa:", sa)
    # print("sb:", sb)

    return ['txt'.join(x.replace(sa, sb, 1).rsplit(x.split('.')[-1], 1)) for x in img_paths_list]
...
class LoadMultiModalImagesAndLabels(Dataset):  # for training/testing
    ...
    def __init__(self, path_rgb, path_ir, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        try:
            ...
            assert (self.img_files_rgb, self.img_files_ir), (
                f'{prefix}No images found', f'{prefix}No images found')
        except Exception as e:
            raise Exception(
                f'{prefix}Error loading data from {path_rgb,path_ir}: {e}\nSee {help_url}')
        ...
        if cache_ir_path.is_file():
            ...
            # changed
            if cache_ir['hash'] != get_hash(self.label_files_ir + self.img_files_ir) or 'version' not in cache_ir:
        ...
        bi_rgb = np.floor(np.arange(n_rgb) /
                          batch_size).astype(int)  # batch index
        ...
        if self.rect:
            ...
            self.batch_shapes_rgb = np.ceil(
                np.array(shapes_rgb) * img_size / stride + pad).astype(int) * stride
            ...
            self.batch_shapes_ir = np.ceil(
                np.array(shapes_ir) * img_size / stride + pad).astype(int) * stride
    ...
    def cache_labels(self, imgfiles, labelfiles, path=Path('./labels.cache'), prefix=''):
        ...
        x = {}  # dict
        nm, nf, ne, nc = 0, 0, 0, 0  # number missing, found, empty, duplicate
        pbar = tqdm(zip(img_files, label_files),
                    desc='Scanning images', total=len(img_files))
        for i, (im_file, lb_file) in enumerate(pbar):
            try:
                # verify images
                im = Image.open(im_file)
    ...
    def __getitem__(self, index):
        ...
        index_rgb = self.indices_rgb[index]
        ...
        if mosaic:
            ...
            # img, labels = load_mosaic(self, index)
            img_rgb, labels_rgb, img_ir, labels_ir = load_mosaic_RGB_IR(
                self, index_rgb, index_ir)
            ...
            # Letterbox
            # final letterboxed shape
            shape = self.batch_shapes_rgb[self.batch_rgb[index]
                                          ] if self.rect else self.img_size
            img_rgb, ratio, pad = letterbox(
                img_rgb, shape, auto=False, scaleup=self.augment)
            img_ir, ratio, pad = letterbox(
                img_ir, shape, auto=False, scaleup=self.augment)
        ...
        if self.augment:
            ...
            # Augment colorspace
            augment_hsv(img_rgb, hgain=hyp['hsv_h'],
                        sgain=hyp['hsv_s'], vgain=hyp['hsv_v'])
            augment_hsv(img_ir, hgain=hyp['hsv_h'],
                        sgain=hyp['hsv_s'], vgain=hyp['hsv_v'])
        ...
        if nL:
            labels_rgb[:, 1:5] = xyxy2xywh(
                labels_rgb[:, 1:5])  # convert xyxy to xywh
        ...
        img_rgb = img_rgb[:, :, ::-1].transpose(2, 0, 1)
    ...
    if (img_rgb is None) and (img_ir is None):  # not cached
        ...
        if r != 1:  # if sizes are not equal
            ...
            img_ir = cv2.resize(img_ir, (int(w0 * r), int(h0 * r)),
                                interpolation=cv2.INTER_AREA if r < 1 and not self.augment else cv2.INTER_LINEAR)
        # img, hw_original, hw_resized
        return img_rgb, img_ir, (h0, w0), img_rgb.shape[:2]
    else:
        # img, hw_original, hw_resized
...
def load_mosaic_RGB_IR(self, index1, index2):
    ...
    # 3 additional image indices
    indices = [index_rgb] + random.choices(self.indices_rgb, k=3)
    ...
    for i, index in enumerate(indices):
        ...
        if i == 0:  # top left
            # base image with 4 tiles
            img4_rgb = np.full(
                (s * 2, s * 2, img_rgb.shape[2]), 114, dtype=np.uint8)
            # base image with 4 tiles
            img4_ir = np.full(
                (s * 2, s * 2, img_ir.shape[2]), 114, dtype=np.uint8)
            # xmin, ymin, xmax, ymax (large image)
            x1a, y1a, x2a, y2a = max(xc - w, 0), max(yc - h, 0), xc, yc
            # xmin, ymin, xmax, ymax (small image)
            x1b, y1b, x2b, y2b = w - (x2a - x1a), h - (y2a - y1a), w, h
        ...
        img4_rgb[y1a:y2a, x1a:x2a] = img_rgb[y1b:y2b,
                                             x1b:x2b]  # img4[ymin:ymax, xmin:xmax]
        img4_ir[y1a:y2a, x1a:x2a] = img_ir[y1b:y2b,
                                           x1b:x2b]  # img4[ymin:ymax, xmin:xmax]
        padw = x1a - x1b
        padh = y1a - y1b
        ...
        labels_rgb, segments_rgb = self.labels_rgb[index].copy(
        ), self.segments_rgb[index].copy()
        labels_ir, segments_ir = self.labels_ir[index].copy(
        ), self.segments_ir[index].copy()
        if labels_rgb.size:
            # normalized xywh to pixel xyxy format
...
def random_perspective_rgb_ir(img_rgb, img_ir, targets_rgb=(), targets_ir=(), segments_rgb=(), segments_ir=(),
    ...
    if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
        if perspective:
            ...
            img_rgb = cv2.warpPerspective(img_rgb, M, dsize=(
                width, height), borderValue=(114, 114, 114))
            img_ir = cv2.warpPerspective(img_ir, M, dsize=(
                width, height), borderValue=(114, 114, 114))
        else:  # affine
            # img = cv2.warpAffine(img, M[:2], dsize=(width, height), borderValue=(114, 114, 114))
            img_rgb = cv2.warpAffine(img_rgb, M[:2], dsize=(
                width, height), borderValue=(114, 114, 114))
            img_ir = cv2.warpAffine(img_ir, M[:2], dsize=(
                width, height), borderValue=(114, 114, 114))
